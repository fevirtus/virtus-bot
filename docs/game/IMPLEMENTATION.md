...
Giữ cấu hình token/database đang dùng. File `.env.example` chỉ là mẫu cho môi trường mới. Không cần biến ID kênh/role của game. `uv run python main.py` chạy code này. Docker Compose mặc định dùng `ghcr.io/fevirtus/virtus-bot:latest`, có thể ghi đè bằng `VIRTUS_IMAGE`. CI kiểm thử trước publish và có tag theo commit; Kubernetes dùng digest cụ thể. Container chạy Python trong virtualenv đã build, không resolve dependency khi khởi động. Không chạy thêm bản bot thứ hai cạnh bản đang hoạt động khi kiểm thử live.

Kết quả kiểm chứng bản 1.0: 35 kiểm thử runtime/Discord mock/PostgreSQL/HTTP và 9 kiểm thử mô hình lịch sử đều đạt; kiểm chứng phát hành cuối ở RELEASE.md. Kiểm tra riêng chạy migration hai lần giữ nguyên bản ghi cấu hình cũ đạt. Compile Python và `git diff --check` đạt. PostgreSQL dùng container tạm riêng, không dùng dữ liệu production. Discord API live đã xác nhận slash commands, kênh/role và bảng persistent trên hai server đủ quyền. Kết quả rollout cuối và server còn thiếu quyền ở RELEASE.md.

## Cập nhật trải nghiệm 2026-09-24 (D033)

- Hồ sơ chỉ giữ trạng thái hiện tại, bước tiếp theo khi đầy tu vi, hoạt động đang chạy; không trộn kết quả trận cũ với bộ tư chất vừa tạo. Nút trực tiếp: Cập nhật, Tư chất & thiên phú, Hướng dẫn, Hộp thư. Điều hướng sửa bảng riêng đang mở; không sửa bảng chung của server.
- Tư chất: giải thích linh căn/ngũ hành, tác dụng từng thiên phú, chỉ số nền chuẩn 100, Cơ duyên/Kiếp số; so sánh hiện tại và bộ mới chưa áp dụng bằng chênh lệch. Không hiển thị Python dictionary hoặc enum tiếng Anh.
- Mỗi người bắt đầu thám hiểm/luyện đan/chiến đấu có một tin hoạt động công khai ở kênh tu tiên. Trạng thái ghi loại hoạt động, mục tiêu và mốc hoàn tất; phó bản có các chặng. Tick sửa tin khi nội dung thay đổi. Lưu ID trong resources; dùng marker lịch sử để đối soát khi gửi thành công nhưng lưu ID bị gián đoạn. DM giữ nguyên cơ chế opt-in.
- Hoàn tất: ghi linh thạch, tu vi thực nhận, vật phẩm và chất lượng, đan chưa dùng trả lại, cống hiến, dấu tích phát hiện. Cập nhật túi và hộp thư trong cùng transaction trước khi gửi Discord; lỗi Discord được thử lại. Hai kết quả gần nhất trên bảng, toàn bộ 30 mục gần nhất trong hộp thư có thời gian.
- Bảng cá nhân không phải đồng hồ tự chạy: bấm Cập nhật hoặc mở lại `/tutien`; bảng công khai tự cập nhật khoảng 30 giây/lần. Nút riêng có thời hạn, bảng mở hồ sơ chung tồn tại qua restart.
- Xác nhận dùng mã riêng theo lần mở xác nhận, không dùng message ID nữa: cùng một bảng riêng có thể mua/luyện nhiều lần mà vẫn chặn click lặp của một xác nhận.
- Kiểm thử hồi quy gồm thám hiểm qua restart không bật DM, đúng thưởng khi đầy tu vi, luyện đan, phó bản tổ đội/kỳ ngộ, retry Discord, phục hồi ID bảng, điều hướng riêng/chung và mã xác nhận độc lập. Số liệu runtime sau sửa Nghịch Thiên lưu riêng, không ghi đè kết quả phát hành 1.0 trước đó.
